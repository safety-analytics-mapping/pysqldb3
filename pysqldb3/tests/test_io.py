...
class TestSQL_to_SQL:
    def set_up(self, from_sql):
        from_sql.drop_table('dbo', '_test_data_io_from_')
        from_sql.query("""
            create table dbo._test_data_io_from_ (
                id_ float,
                name varchar(20),
                dte datetime
            );
            insert into dbo._test_data_io_from_ 
                (id_, name, dte)
            values 
                (1, 'testing', '2024-10-21 00:00:00'), 
                (2, '2testing', '2024-10-20 00:00:00');
        """)
        assert from_sql.table_exists('_test_data_io_from_', schema='dbo')

    def test_sql_to_sql_basic(self):
        from_sql = sql
        to_sql = sql
        self.set_up(from_sql)
        assert from_sql.table_exists('_test_data_io_from_', schema='dbo')
        to_sql.drop_table('dbo', '_test_data_io_to_')
        assert not to_sql.table_exists('_test_data_io_to_', schema='dbo')

        data_io.sql_to_sql(from_sql, to_sql, '_test_data_io_from_', org_schema='dbo', dest_schema='dbo', dest_table='_test_data_io_to_', spatial=False)
        assert to_sql.table_exists('_test_data_io_to_', schema='dbo')

        fdf = from_sql.dfquery("select * from dbo._test_data_io_from_")
        tdf = to_sql.dfquery("select * from dbo._test_data_io_to_")
        tdf = tdf.drop(['ogr_fid'], axis=1)
        assert  fdf.equals(tdf)

        from_sql.drop_table('dbo', '_test_data_io_from_')
        to_sql.drop_table('dbo', '_test_data_io_to_')
        assert not to_sql.table_exists('_test_data_io_to_', schema='dbo')

    def test_sql_to_sql_basic_query(self):
        from_sql = sql
        to_sql = sql
        self.set_up(from_sql)
        assert from_sql.table_exists('_test_data_io_from_', schema='dbo')
        ...
        data_io.sql_to_sql_qry(from_sql, to_sql, 'select id_, name from _test_data_io_from_', org_schema='dbo', dest_schema='dbo', dest_table='_test_data_io_to_', spatial=False)
        assert to_sql.table_exists('_test_data_io_to_', schema='dbo')

        fdf = from_sql.dfquery("select id_, name from dbo._test_data_io_from_")
        tdf = to_sql.dfquery("select * from dbo._test_data_io_to_")
        tdf = tdf.drop(['ogr_fid'], axis=1)
        assert  fdf.equals(tdf)
    ...
    def test_sql_to_sql_basic_geom(self):
        from_sql = sql
        to_sql = sql
        self.set_up(from_sql)
        from_sql.query("alter table dbo._test_data_io_from_ add Shape geometry;", timeme=False)
        from_sql.query("update dbo._test_data_io_from_ set Shape = geometry::STGeomFromText('POINT (912935.68103027344 119003.59259033203)',2263)", timeme=False)
        assert from_sql.table_exists('_test_data_io_from_', schema='dbo')
        to_sql.drop_table('dbo', '_test_data_io_to_')
        assert not to_sql.table_exists('_test_data_io_to_', schema='dbo')

        data_io.sql_to_sql(from_sql, to_sql, '_test_data_io_from_', org_schema='dbo', dest_schema='dbo', dest_table='_test_data_io_to_', spatial=True)
        assert to_sql.table_exists('_test_data_io_to_', schema='dbo')

        fdf = from_sql.dfquery("select Shape, id_, name, dte from dbo._test_data_io_from_")
        tdf = to_sql.dfquery("select * from dbo._test_data_io_to_")
        tdf = tdf.drop(['ogr_fid'], axis=1)
        assert  fdf.equals(tdf)
