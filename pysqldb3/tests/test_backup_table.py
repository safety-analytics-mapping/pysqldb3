...
class TestBackupTablesPg:
    ...
    def test_backup_tables_capitalized(self):
        db.drop_table(table=test_pg_to_backup.capitalize(), schema=pg_schema)

        # table schema
        db.query(f"""
            CREATE TABLE {pg_schema}.{test_pg_to_backup.capitalize()} (
                id int,
                column2 text,
                column3 timestamp,
                "1 test messy column" text
        ...
        for i in range(10):
            db.query(f"""
                INSERT INTO {pg_schema}.{test_pg_to_backup.capitalize()}
                    (id, column2, column3, "1 test messy column")
                values ({i}, '{i}', '{'2022-10-14 10:12:40-04'}', '{'test '*i}')
            """)
        # validate table created
        assert db.table_exists(test_pg_to_backup.capitalize(), schema=pg_schema)
        db.query(f"select count(*) cnt from {pg_schema}.{test_pg_to_backup.capitalize()}")
        assert db.data[0][0] == 10
        ...
        db.backup_table(pg_schema, test_pg_to_backup.capitalize(), test_back_file, pg_schema, test_pg_from_backup.capitalize())
        assert os.path.isfile(test_back_file)

        # run backup
        db.drop_table(pg_schema, test_pg_from_backup.capitalize())
        schema_table_name = db.create_table_from_backup(test_back_file, case_sensitive = True)

        # validate table exists
        assert db.table_exists(test_pg_from_backup.capitalize(), schema=pg_schema)
        db.query(f"select count(*) cnt from {pg_schema}.{test_pg_from_backup.capitalize()}")
        assert db.data[0][0] == 10

        # Validate schema matches
        _to = db.get_table_columns(test_pg_to_backup.capitalize(), schema=pg_schema)
        _from = db.get_table_columns(test_pg_from_backup.capitalize(), schema=pg_schema)
        assert _to == _from

        # clean up
        db.cleanup_new_tables()
        assert not db.table_exists(test_pg_to_backup.capitalize(), schema=pg_schema, case_sensitive = True)
        assert not db.table_exists(test_pg_from_backup.capitalize(), schema=pg_schema, case_sensitive = True)
        if os.path.isfile(test_back_file):
            os.remove(test_back_file)
        assert not os.path.isfile(test_back_file)    
...
class TestBackupTablesMs:
    ...
    def test_backup_tables_capitalized(self):
        sql.drop_table(table=test_sql_to_backup.capitalize(), schema=ms_schema)

        # table schema
        sql.drop_table(ms_schema, test_sql_to_backup.capitalize())
        sql.query(f"""
            CREATE TABLE {ms_schema}.{test_sql_to_backup.capitalize()} (
                id int,
                column2 text,
                column3 datetime,
                "1 test messy column" text
        ...
        for i in range(10):
            sql.query(f"""
                INSERT INTO {ms_schema}.{test_sql_to_backup.capitalize()}
                    (id, column2, column3, "1 test messy column")
                values ({i}, '{i}', '{'2022-10-14 10:12:40'}', '{'test '*i}')
            """)
        # validate table created
        assert sql.table_exists(test_sql_to_backup.capitalize(), schema=ms_schema)
        sql.query(f"select count(*) cnt from {ms_schema}.{test_sql_to_backup.capitalize()}")
        assert sql.data[0][0] == 10
        ...
        sql.backup_table(ms_schema, test_sql_to_backup.capitalize(), test_back_file, ms_schema, test_sql_from_backup.capitalize())
        assert os.path.isfile(test_back_file)
        ...
        # Validate schema matches
        _to = sql.get_table_columns(test_sql_to_backup.capitalize(), schema=ms_schema)
        _from = sql.get_table_columns(test_sql_from_backup.capitalize(), schema=ms_schema)
        assert _to == _from

        # clean up
        sql.cleanup_new_tables()
        assert not sql.table_exists(test_sql_to_backup.capitalize(), schema=ms_schema)
