...
class TestBackupTablesPg:
    ...
    def test_backup_tables_capitalized(self):
        ...
        db.backup_table(pg_schema, test_pg_to_backup, test_back_file, pg_schema, test_pg_from_backup.capitalize())
        assert os.path.isfile(test_back_file)
        ...
        # validate table exists
        assert db.table_exists(test_pg_from_backup.capitalize(), schema=pg_schema, case_sensitive = True)
        ...
        # clean up
        db.cleanup_new_tables()
        assert not db.table_exists(test_pg_to_backup, schema=pg_schema, case_sensitive = True)
