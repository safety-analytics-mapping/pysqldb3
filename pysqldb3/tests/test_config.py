...
cfg_data_default = {'ODBC Drivers': ['ODBC_DRIVER', 'NATIVE_DRIVER'],
                    'DEFAULT DATABASE': ['type','server','database'],
                    'GDAL DATA': ['GDAL_DATA']}
testing_keys = ['TYPE','SERVER','DB_NAME','DB_USER','DB_PASSWORD']
cfg_data_testing = {'PG_DB': testing_keys, 'SECOND_PG_DB': testing_keys, 'SQL_DB': testing_keys, 'SECOND_SQL_DB': testing_keys}
...
class TestConfigIOInternal:

    def test_read_default_config(self):
        # Test
        # Fn seems to be working, but its returning an empty dict
        cfg = read_config(config_file)

        # Assert correct sections
        assert cfg.keys() == cfg_data_default.keys()
        
        # Assert correct keys in each section
        for sec in cfg_data_default:
            assert cfg.get(sec) == cfg_data_default.get(sec)
    ...
    def test_write_default_config(self):
        ...
        assert cfg_sections.keys() == cfg_data_default.keys()
        for sec in cfg_data_default:
            assert cfg_sections.get(sec) == cfg_data_default.get(sec)
...
class TestConfigIOConfigParser:
    @classmethod
    def setup(cls):
        cls.cp = ConfigParser()

    def test_read_default_config_configparser(self):
        # Test ConfigParser.read() and assert it reads in cfg file
        assert self.cp.read(config_file) == ['config.cfg']

        # assert correct sections read in
        assert self.cp.sections() == cfg_data_default.keys()

        # assert correct keys in sections
        for sec in self.cp.sections():
            assert self.cp[sec] == cfg_data_default.values()

    def test_write_default_config_configparser(self):
        pass

    def test_read_testing_config_configparser(self):
        pass

    def test_write_testing_config_configparser(self):
        pass
