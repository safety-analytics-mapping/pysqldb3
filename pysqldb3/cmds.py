...
user={username} password={password}" "{shpfile}" -nln {schema}.{table} {precision}
""".replace('\n', ' ')
...
"EPSG:{srid}" -f MSSQLSpatial "MSSQL:server={host}; database={dbname}; UID={username}; PWD={password}"
 "{gdb}" "{feature}" -nln {schema}.{table} -progress --config MSSQLSPATIAL_USE_GEOMETRY_COLUMNS NO {shpfile_name}
""".replace('\n', ' ')
...
{pg_schema}.{pg_table} -lco OVERWRITE=yes -nln {ms_schema}.{ms_table} {spatial} {nlt_spatial} -progress 
--config MSSQLSPATIAL_USE_GEOMETRY_COLUMNS NO
""".replace('\n', ' ')
...
-nln {pg_schema}.{pg_table} {nlt_spatial} -progress --config MSSQLSPATIAL_LIST_ALL_TABLES YES
""".replace('\n', ' ')
...
UID={ms_username}; PWD={ms_password}" -sql "{sql_select}" -lco OVERWRITE=yes -nln {pg_schema}.{pg_table} {nlt_spatial} 
-progress --config MSSQLSPATIAL_LIST_ALL_TABLES YES
""".replace('\n', ' ')

SQL_TO_PG_LDAP_CMD = r"""
ogr2ogr --config GDAL_DATA "{gdal_data}" -overwrite -f "PostgreSQL"
PG:"host={pg_host} port={pg_port} dbname={pg_dbname} user={pg_username} password={pg_password}" -f {spatial} 
"MSSQL:server={ms_host};database={ms_dbname};" {ms_schema}.{ms_table} -nln {pg_schema}.{pg_table} {nlt_spatial} -progress 
--config MSSQLSPATIAL_LIST_ALL_TABLES YES
""".replace('\n', ' ')
...
ogr2ogr --config GDAL_DATA "{gdal_data}" -overwrite -f "PostgreSQL" 
PG:"host={pg_host} port={pg_port} dbname={pg_dbname} user={pg_username} password={pg_password}" 
-f {spatial} "MSSQL:server={ms_host};database={ms_dbname};UID={ms_username};PWD={ms_password}" {ms_schema}.{ms_table} 
-lco OVERWRITE=yes  -nln {pg_schema}.{pg_table} {nlt_spatial} -progress --config MSSQLSPATIAL_LIST_ALL_TABLES YES
""".replace('\n', ' ')
...
dbname={dest_dbname} user={dest_username} password={dest_password}" PG:"host={src_host} port={src_port} 
dbname={src_dbname}  user={src_username} password={src_password}" {src_schema}.{src_table} 
-lco OVERWRITE=yes -nln {dest_schema}.{dest_table} {nlt_spatial} -progress
""".replace('\n', ' ')
