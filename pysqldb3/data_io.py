...
def sql_to_pg(ms, pg, orig_table_name, LDAP=False, spatial=True, orig_schema_name=None, dest_schema_name=None, print_cmd=False,
              dest_table_name=None, temp=True, gdal_data_loc=GDAL_DATA_LOC, pg_encoding='UTF8'):
    """
    ...
    """
    if not orig_schema_name:
        orig_schema_name = ms.default_schema
    ...
    if LDAP:
        cmd = SQL_TO_PG_LDAP_CMD.format(
            ...
            pg_database=pg.database,
            pg_schema=dest_schema_name,
            pg_table=dest_table_name,
            ms_table=orig_table_name,
            ms_schema=orig_schema_name,
            spatial=spatial,
            to_pg_name=dest_table_name,
            nlt_spatial=nlt_spatial
        )
    else:
...
def pg_to_pg(from_pg, to_pg, orig_table_name, orig_schema_name=None, dest_schema_name=None, print_cmd=False, dest_table_name=None,
    ...
    :param orig_table_name: Source table name
    :param orig_schema_name: PostgreSQL schema for origin table (defaults to default schema)
    ...
    """
    if not orig_schema_name:
        orig_schema_name = from_pg.default_schema

    if not dest_schema_name:
        dest_schema_name = to_pg.default_schema
    ...
    cmd = PG_TO_PG_CMD.format(
        ...
        from_pg_schema=orig_schema_name,
        to_pg_schema=dest_schema_name,
        from_pg_table=orig_table_name,
        to_pg_name=dest_table_name,
        nlt_spatial=nlt_spatial,
        gdal_data=GDAL_DATA_LOC
    )
    ...
    clean_geom_column(to_pg, dest_table_name, dest_schema_name)

    to_pg.tables_created.append(dest_schema_name + "." + dest_table_name)

    if temp:
        to_pg.log_temp_table(dest_schema_name, dest_table_name, to_pg.user)


def pg_to_pg_qry(from_pg, to_pg, query, dest_schema_name=None, print_cmd=False, dest_table_name=None,
             spatial=True, temp=True):
    """
    ...
    if not dest_table_name:
        dest_table_name = '_{u}_{d}'.format(u=to_pg.user, d=datetime.datetime.now().strftime('%Y%m%d%H%M'))
